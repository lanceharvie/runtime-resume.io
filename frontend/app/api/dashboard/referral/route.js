...
import { getPipelineDashboardReferral, isPipelineApiConfigured } from "@/lib/pipeline-api"
...
export async function GET(request) {
  try {
    ...
    const data = await getPipelineDashboardReferral(sessionToken)
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to load dashboard referral" },
      { status: 400 }
    )
  }
}
