...
import { getPipelineDashboardSession, isPipelineApiConfigured } from "@/lib/pipeline-api"
...
export async function GET(request) {
  try {
    ...
    const data = await getPipelineDashboardSession(sessionToken)
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to load dashboard session" },
      { status: 401 }
    )
  }
}
