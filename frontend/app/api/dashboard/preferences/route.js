...
import { isPipelineApiConfigured, updatePipelineDashboardPreferences } from "@/lib/pipeline-api"
...
export async function PATCH(request) {
  try {
    ...
    const data = await updatePipelineDashboardPreferences(sessionToken, payload)
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to update dashboard preferences" },
      { status: 400 }
    )
  }
}
